...
define_page_mode("hackernews_comments_mode",
    ...
    function enable (buffer) {
        ...
        if (buffer.browser.webProgress.isLoadingDocument)
            /* arrange so that _on_hackernews_load callback is called once
               the document is done loading. */
            add_hook.call(buffer, "buffer_loaded_hook", _on_hackernews_comments_load);
        else
            /* call it right now in case the page-mode document is already
             * loaded (ie, someone M-x hackernews-mode manually) */
